...
| UC12 | Entrar no sistema | Anônimo | 🟡 | POST `/auth/login` | ServicoCliente | **Pessoa 2 (PR #4):** authenticates by email+senha; uniform `Credenciais inválidas` (no enumeration); `senha` omitted from response. Verified working. (Session/token deferred to P5 auth; returns 400 not 401 on failure — review note) |
