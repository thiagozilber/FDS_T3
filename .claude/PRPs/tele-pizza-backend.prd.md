...
| P3 | 17/06/2026 | Persistência com JPA | Migrate JDBC repos → JPA; entities annotated | P2 | **Pessoa 2 (framework) · ambos (repos próprios)** — Seam #3 | 🟡 in-progress — **Pessoa 2 (PR #4):** `Cliente`→JPA (Approach A). **Pessoa 1 ✅ (`feat/p3-jpa-pessoa1`):** `Pedido*`/`Historico`/`Estoque`→JPA via separate persistence model (Approach B — keeps Pessoa 2's entities out of JPA; see §13, 28/06 P3 impl) — 90/90 green, full lifecycle live-verified. Remaining: Pessoa 2's cardápio/desconto repos — [plan](plans/jpa-migration-pessoa1-p3.plan.md) |
...
2. ~~**JPA vs pure-POJO domain.**~~ **Resolved (P3 — Seam #3, 2026-06-28):** **Approach A** for the flat `Cliente` (Pessoa 2, PR #4); **Approach B** (separate persistence model, pure-POJO domain) for Pessoa 1's `Pedido` aggregate, because annotating it would drag Pessoa 2's `Produto`/`Receita`/`Ingrediente` into JPA (see §13 "P3 impl — Pessoa 1", 28/06). `ddl-auto: none` + literal physical naming strategy.
