...
- **Created:** 2026-06-08 · **Last updated:** 2026-06-28
...
| UC9 | Pagar pedido | Cliente | ✅ | POST `/pedidos/{id}/pagar` | ServicoPagamento (fake), Cozinha (sim), Entrega (sim), ServicoPedido | **Pessoa 1 done:** fake-pay → PAGO (stamped) → AGUARDANDO → handoff to `ICozinhaService`; each transition persisted (Seam #2). **Pessoa 2 done (PR #4):** real `CozinhaService`/`EntregaService` sims now advance PREPARACAO→PRONTO→TRANSPORTE→ENTREGUE via `IRegistradorStatus` callbacks — full lifecycle verified end-to-end |
...
| UC11 | Cadastrar usuário | Anônimo | 🟡 | POST `/clientes` | ServicoCliente + ClienteRepository | **Pessoa 2 (PR #4):** registers cliente (CPF/email unique, `senha` column added); validated via `@Valid`; `senha` omitted from response. Verified working. ⚠️ pending review fixes (see §14, 28/06) before merge |
| UC12 | Entrar no sistema | Anônimo | ✅ | POST `/auth/login` | ServicoCliente, RepositorioSessao | **Pessoa 2:** authenticates by email+senha; uniform `Credenciais inválidas` (no enumeration) → **401** (fixed); `senha` omitted. **P5 (29/06):** now issues an opaque Bearer token + `papel`; also authenticates the config admin |
...
| **Cozinha** | ✅ | `ICozinhaService`/`CozinhaService` | **Pessoa 2 (PR #4):** now `@Service`, persists PREPARACAO/PRONTO via `IRegistradorStatus` (Seam #2), then hands off to `EntregaService`. `CozinhaServiceStub` deleted (Seam #1) |
| **Entrega** | ✅ | `IEntregaService`/`EntregaService` | **Pessoa 2 (PR #4):** mirrors Cozinha (queue + `ScheduledExecutorService`); persists TRANSPORTE→ENTREGUE via `IRegistradorStatus` (Seam #2) |
| **Cliente** | 🟡 | `ServicoCliente` + `ClienteRepository` | **Pessoa 2 (PR #4):** cadastro (CPF/email unique) + autenticação (UC11/UC12). ⚠️ pending review fixes (§14, 28/06) |
...
| **P2** | **15/06/2026** | **Ciclo do pedido** | **UC6, UC7, UC8, UC9** (+ Pedidos, Estoque, Pagamento, Entrega services) | P1 | **Pessoa 1 (lead) · Pessoa 2 (sims Cozinha/Entrega)** — Seams #1, #2 | **✅ complete** — Pessoa 1 ✅ (UC6–UC9 flow) · Pessoa 2 ✅ (Cozinha/Entrega sims, PR #4) — full lifecycle verified |
| P3 | 17/06/2026 | Persistência com JPA | Migrate JDBC repos → JPA; entities annotated | P2 | **Pessoa 2 (framework) · ambos (repos próprios)** — Seam #3 | ✅ complete — **unified Approach A** (annotated domain entities, no separate persistence model). **Pessoa 2:** `Cliente` (PR #4) + `Cardapio`/`Produtos`/`Ingredientes`/`Receitas` (skad) → JPA. **Pessoa 1:** `Pedido*`/`Historico`/`Estoque` → JPA; the interim Approach-B persistence model was converted to Approach A on `feat/jpa-approach-a-unified` (see §13, 28/06 "Approach B→A conversion"). **135/135 green + full lifecycle live-verified.** |
| P4 | 22/06/2026 | Usuários + histórico | UC10, UC11, UC12 | P3 | **Pessoa 2 (UC11/UC12) · Pessoa 1 (UC10)** — Seam #4 | ✅ complete — Pessoa 1 ✅ (UC10) · Pessoa 2 ✅ (UC11/UC12; review fixes applied: 401-on-login, dup-bean/ddl-auto, archives removed) |
| P5 | 24/06/2026 | Autenticação | Login/authorization enforced on `(A)`/`(Adm)` UCs | P4 | **Pessoa 2** | ✅ complete — [plan](plans/p5-authentication.plan.md). Token-Bearer interceptor + config admin (OQ#5). Anon→401, cliente→admin route→403; 146/146 tests + live smoke on :8099 |
| P6 | 29/06/2026 | Drivers de teste | Tests (with comment-form specs) for: discount strategies, ServicoDesconto, ServicoImposto, SubmeterPedidoUC | P1–P5 | **dividido — Pessoa 1 (Pedido/Estoque) · Pessoa 2 (Desconto/Imposto/Auth)** | ✅ complete — Pessoa 1 ✅ · Pessoa 2 ✅ (Imposto drivers `Casos de teste` blocks added 29/06; Desconto/Auth already done) — [plan](plans/completed/test-drivers-pessoa1-p6.plan.md) · [report](reports/test-drivers-pessoa1-p6-report.md) |
...
- _2026-06-28 (Seam #3 / OQ#2 — JPA framework decision, P3 — Pessoa 2, PR #4)_: **G1 — chose Approach A: annotate the domain entities directly** (`@Entity/@Table/@Id/@Column` + Lombok `@NoArgsConstructor` on `Cliente`), accepting the documented tension with the "pure-POJO `Dominio.Entidades`" rule (§4/§6.1) rather than introducing a separate persistence model. **G3 — `spring-boot-starter-data-jpa` added** to `pom.xml`. `ClienteRepositoryJPA` implements the unchanged `ClienteRepository` port via `EntityManager` (DIP preserved; not Spring Data `JpaRepository`). **G2 — `ddl-auto` / init policy: ⚠️ NOT YET SET — must be `spring.jpa.hibernate.ddl-auto: none`** so `schema.sql` keeps owning DDL; without it Hibernate defaults to `create-drop` on embedded H2 and drops/recreates `clientes` after `data.sql`, wiping the seeded clients (confirmed in the 28/06 review). **This resolves OQ#2** for Pessoa 2's repos; Pessoa 1's `Pedido*`/`Estoque` migration remains pending against this same Approach A.
- _2026-06-28 (Auth impl — Pessoa 2, PR #4)_: **D16** — UC11/UC12 land as `ServicoCliente.cadastrar`/`autenticar`. CPF is the `Cliente` PK; email uniqueness checked in-service before insert (no DB `UNIQUE(email)` yet — review note). `autenticar` returns a uniform `IllegalArgumentException("Credenciais inválidas.")` for both unknown-email and wrong-password (no user enumeration) → currently mapped to **400** by the global advice (should be **401** — review note). `senha` is excluded from `CadastrarClienteResponse`/`LoginResponse`. Password hashing intentionally **out of scope** for this Clean-Architecture/SOLID assignment (plaintext accepted).
- _2026-06-28 (P3 impl — Pessoa 1)_: **Approach B for the Pedido aggregate (ratified deviation from the Approach-A decision above, for this slice only).** Annotating the domain `Pedido`/`ItemPedido`/`ItemEstoque` (Approach A) would force JPA `@ManyToOne` onto Pessoa 2's still-JDBC `Produto`/`Receita`/`Ingrediente` — crossing Seam #3. Instead the domain entities stay **pure POJOs** and Pessoa 1's repos use **separate persistence entities** (`PedidoJpaEntity`, `ItemPedidoJpaEntity` + `ItemPedidoId`, `HistoricoStatusJpaEntity`, `ItemEstoqueJpaEntity` in `Adaptadores/Dados/Jpa`) mapped **by FK id**; the adapters (`PedidoRepositoryJPA`/`HistoricoStatusRepositoryJPA`/`ItensEstoqueRepositoryJPA`, via `EntityManager`) translate entity↔domain and re-hydrate `Produto` through the existing `ProdutosRepository`. The 3 `*RepositoryJDBC` were deleted (one adapter per port — the PR #4 duplicate-bean trap). **Config:** added `spring.jpa.hibernate.naming.physical-strategy=PhysicalNamingStrategyStandardImpl` so `@Table`/`@Column` names are literal — required by the camelCase `itensEstoque` table (Hibernate's default rewrote it to `itens_estoque` → "table not found"; caught only by a live smoke, not the suite). `baixaSeDisponivel` kept **atomic** (bulk JPQL `UPDATE … WHERE quantidade >= :q`); `atualizaStatus`/`atualizaDataHoraPagamento` use **load-mutate** (not bulk) to avoid stale L1-cache reads in `submeter`'s final `recuperaPorId`. `recuperaPorId`→`null` 404 contract preserved. **`preco bigint`-vs-`int` (OQ#6c) stays deferred** — it lives in Pessoa 2's `ProdutosRepositoryJDBC`, out of this slice's scope (Approach B leaves `Produto` on JDBC). Domain ports unchanged; **90/90 tests green (no test edits)**; full lifecycle (submit→pay→PREPARACAO→PRONTO→TRANSPORTE→ENTREGUE), UC8 cancel+devolve, UC10 entregues, and 400/404 paths live-verified on :8099 under JDK 21.

- _2026-06-28 (Approach B→A conversion — Pessoa 1, `feat/jpa-approach-a-unified`)_: **Reverted the interim Approach B and migrated the `Pedido` aggregate + `ItemEstoque` to Approach A** (annotated domain entities), unifying the whole project on Approach A (the original G1 decision). This became clean once Pessoa 2's `Produto`/`Receita`/`Ingrediente` were JPA entities (skad menu migration), removing the Seam #3 obstacle that justified Approach B. **Changes:** `Pedido` `@Entity` (`@Id @GeneratedValue(IDENTITY)`, `@ManyToOne Cliente`, `@OneToMany(mappedBy="pedido", cascade=ALL, orphanRemoval, EAGER) itens`, `@Enumerated(STRING)`); `ItemPedido` `@Entity @IdClass(ItemPedidoId)` with the composite key as **derived identity** via `@ManyToOne pedido` + `@ManyToOne Produto item`; `ItemEstoque` `@Entity` (surrogate `@Id` + `@ManyToOne Ingrediente`). Adapters (`PedidoRepositoryJPA`/`ItensEstoqueRepositoryJPA`) now persist/read the domain entities directly — **mapper layer deleted** (`PedidoJpaEntity`/`ItemPedidoJpaEntity`/`ItemEstoqueJpaEntity`/old `Jpa/ItemPedidoId`), and `PedidoRepositoryJPA` no longer depends on `ProdutosRepository` for rehydration. `HistoricoStatusJpaEntity` **kept** (no domain entity exists for `historico_status`). **Preserved:** domain ports unchanged, atomic `baixaSeDisponivel`, `recuperaPorId`→`null` 404, UC10 ENTREGUE-window query, load-mutate status updates, bad-cpf→400 (FK violation at flush). **No `@ToString`** on `Pedido` — the async Cozinha/Entrega sims print the order off-session, so default `toString` avoids `LazyInitializationException`. Verified JDK 21: **135/135 tests green (no test edits)** + live HTTP smoke (full lifecycle → ENTREGUE, UC8 cancel+devolve, UC10, 404/400). **`preco bigint`-vs-`int` (OQ#6c)** still deferred (lives in `Produto`/`ProdutosRepository`).
...
2. ~~**JPA vs pure-POJO domain.**~~ **Resolved → unified Approach A (P3, 2026-06-28):** all entities annotated directly (`Cliente`; `Produto`/`Receita`/`Ingrediente`; `Pedido`/`ItemPedido`/`ItemEstoque`) — **no separate persistence model**. The interim Approach-B for the `Pedido` aggregate (recorded below) was reverted once `Produto`/`Receita`/`Ingrediente` became JPA entities (skad), so the aggregate now maps via real `@ManyToOne` associations + a `@OneToMany` cascade (see the 2026-06-28 "Approach B→A conversion" decision below). `ddl-auto: none` + literal physical naming strategy (`PhysicalNamingStrategyStandardImpl`, required by the camelCase `itensEstoque`).
...
5. ~~**Auth mechanism.** Spring Security vs lightweight token filter.~~ **Resolved (P5, 2026-06-29):** chose the **lightweight token filter** (no Spring Security dependency) — a `HandlerInterceptor` (`AutenticacaoInterceptor`) validating an opaque `Authorization: Bearer <UUID>` token against an in-memory `RepositorioSessao` port (`RepositorioSessaoMemoria`, no expiry, resets on boot). Roles: **config-based single admin** (`app.admin.*`, `ROLE`=ADMIN, not a `clientes` row) vs customers (`ClienteRepository`, CLIENTE). UC12 `/auth/login` now issues the token. Authorization matrix: public = `/`, `POST /auth/login`, `POST /clientes`, `/h2/**`; ADMIN = `/cardapio/**` (except `GET /cardapio/corrente`) + `/descontos/**`; authenticated = `GET /cardapio/corrente` + `/pedidos/**`. Plaintext senha kept (D16). See §14 (29/06).
...
- **2026-06-29** — **P6 ✅ COMPLETE + P4 ✅ confirmed (branch `feat/p5-authentication`).** Added the spec-required in-file `Casos de teste` comment blocks to Pessoa 2's four Imposto drivers (`ServicoImpostoTest`, `FabricaEstrategiaImpostoTest`, `Lei0412de2022Test`, `Lei5762de2026Test`) — tests already existed and passed; only the comment-form specs were missing (Desconto/`ServicoCliente`/auth already had them). **P6 → ✅** (Pessoa 1 + Pessoa 2). **P4 → ✅** (UC11/UC12 review fixes confirmed applied: 401-on-login, dup-bean/`ddl-auto: none`, stray archives removed). No code/behavior change; doc-in-test only. Remaining besides the **demo (P7)**: optional polish only — `UNIQUE(email)` in `schema.sql` (in-service today), `preco bigint`-vs-`int` (OQ#6c), lock down H2 console + DEBUG logging (e-sec). Committed onto PR #8.
- **2026-06-29** — **P5 ✅ COMPLETE — Autenticação & Autorização (branch `feat/p5-authentication`, off `main`).** Implemented the **lightweight token interceptor** (OQ#5 resolved → §13.5; the Spring Security draft was tuned down per user direction). New: `Dominio/Entidades/Papel` (ADMIN/CLIENTE); `Aplicacao/Portas/{Sessao,RepositorioSessao}` (port, DIP); `Adaptadores/Seguranca/{RepositorioSessaoMemoria,AutenticacaoInterceptor}` (in-memory opaque-UUID tokens; `preHandle` → 401 missing/unknown token, 403 customer-on-admin-route, OPTIONS passthrough for CORS); `Adaptadores/Config/{AdminProperties,WebConfig}` (`app.admin.*` config admin; interceptor registration excluding `/`,`/auth/login`,`/clientes`,`/h2/**`,`/error`). `LoginUC`/`LoginResponse` now issue a Bearer token + `papel` (admin via config OR cliente via `ServicoCliente.autenticar`); `senha` never returned. **No new Maven dependency.** Authorization matrix: ADMIN=`/cardapio/**` (except `GET /cardapio/corrente`)+`/descontos/**`; authenticated=`GET /cardapio/corrente`+`/pedidos/**`. **UC12 ✅** (token issuance). **Convention fixes during impl:** the WIP files lacked packages/imports, had a `Map<String,sessao>` typo + missing `@Component`, and sat under a non-ASCII `Segurança/` dir — corrected and relocated to ASCII `Seguranca/` (matches `Aplicacao`/`Dominio` naming). **Test (P6 Pessoa-2 auth driver):** `AutenticacaoIntegrationTest` (11 cases, in-file `Casos de teste` block) — uses plain `@SpringBootTest` + `webAppContextSetup` (NOT `@AutoConfigureMockMvc`, which forked a 2nd context and re-ran `data.sql` → PK violation on the shared in-mem H2). **Verified JDK 21: 146/146 green (135 + 11)** and **live HTTP smoke on :8099** — anon→401, `/`→200, cliente login→token, cliente `GET /cardapio/corrente`→200 / `/pedidos/1/status`→404 / `/descontos/politicas`→403, admin login→token(papel ADMIN, no senha)/`/descontos/politicas`→200/`/cardapio/lista`→200, bad token→401. **Not committed to `main`** (work on `feat/p5-authentication`). Follow-ups: per-customer ownership scoping on `/pedidos/*`+UC10, token expiry/logout, password hashing, lock down H2+DEBUG (OQ#6c/e-sec), `UNIQUE(email)` in schema.
- **2026-06-28** — **P3 ✅ COMPLETE — unified Approach A (branch `feat/jpa-approach-a-unified`, off `skad-branch` + `main` merged).** Integrated Pessoa 2's skad menu-JPA migration with `main` and finished P3 in two passes. **Pass 1 (review fixes):** `ServicoClienteTest` now asserts `CredenciaisInvalidasException` (the text-clean skad+main merge had left it asserting the pre-PR#6 `IllegalArgumentException` → 2 failures); `CardapioRepositoryJPA.indicacoesDoChef` returns `[]` instead of `List.of(null)` (NPE) when produto 2 is absent, + a unit test for that branch. **Pass 2 (Approach B→A):** converted Pessoa 1's `Pedido`/`ItemPedido`/`ItemEstoque` from the interim separate-persistence-model (Approach B) to **annotated domain entities** (Approach A), unifying the project on Approach A now that `Produto`/`Receita`/`Ingrediente` are JPA entities (skad). Composite `itens_pedido` key via `@IdClass` derived identity (`@ManyToOne pedido` + `@ManyToOne Produto`); `@OneToMany(cascade) itens`; mapper layer (`*JpaEntity` + old `ItemPedidoId`) deleted; `HistoricoStatusJpaEntity` kept (no domain entity for `historico_status`). Domain ports unchanged; atomic `baixaSeDisponivel`, `recuperaPorId`→null 404, UC10 query, bad-cpf→400 all preserved; no `@ToString` (async sims print off-session). See §13 (28/06 "Approach B→A conversion") + OQ#2. **Verified JDK 21: 135/135 tests green (no test edits, incl. `@SpringBootTest` ITs + `contextLoads`)** and **live HTTP smoke on :8099** — submit→APROVADO→pay→PREPARACAO→PRONTO→TRANSPORTE→ENTREGUE, UC10 entregues, UC8 cancel+devolve, and 404/400 (FK bad-cpf / unknown-produto / empty-itens). Status: **P3 ✅ complete** (all repos JPA, Approach A). Follow-ups unchanged: `preco bigint`-vs-`int` (OQ#6c), `UNIQUE(email)` in schema, P5 auth.

- **2026-06-28** — **P3 IMPLEMENTED (Pessoa 1 slice) — `Pedido*`/`Historico`/`Estoque` JDBC→JPA.** On `feat/p3-jpa-pessoa1` (off `main` after PR #4 merged). Migrated the three Pessoa-1 adapters to JPA via **Approach B** (separate persistence model — domain entities stay pure POJOs): new `PedidoJpaEntity`/`ItemPedidoJpaEntity`(+`ItemPedidoId`)/`HistoricoStatusJpaEntity`/`ItemEstoqueJpaEntity` in `Adaptadores/Dados/Jpa`, and `PedidoRepositoryJPA`/`HistoricoStatusRepositoryJPA`/`ItensEstoqueRepositoryJPA` (`EntityManager`) implementing the **unchanged** domain ports; the 3 `*RepositoryJDBC` deleted (one adapter per port). **Chose Approach B over the recorded Approach A** because the aggregate references Pessoa 2's still-JDBC `Produto`/`Ingrediente` (annotating would cross Seam #3) — confirmed with the user (see §13, 28/06 P3 impl). **Config:** `spring.jpa.hibernate.naming.physical-strategy=PhysicalNamingStrategyStandardImpl` (literal names — fixes the camelCase `itensEstoque` mapping; a live-smoke-only catch). Preserved: atomic `baixaSeDisponivel`, `recuperaPorId`→null 404, UC10 ENTREGUE-window JOIN. **`./mvnw test` 90/90 green with no test edits** (incl. the `@SpringBootTest` H2 IT + `contextLoads`); **live HTTP smoke on :8099 (JDK 21)** verified the full lifecycle (submit→pay→PREPARACAO→PRONTO→TRANSPORTE→ENTREGUE), UC8 cancel+devolve, UC10 entregues, and 400/404 paths. **`preco bigint`-vs-`int` (OQ#6c) left deferred** — lives in Pessoa 2's `ProdutosRepositoryJDBC`, out of scope under Approach B. **Not committed to `main`** (work on the feature branch). Remaining P3: Pessoa 2's cardápio/desconto repos.

- **2026-06-28** — **PR #4 (Pessoa 2) reviewed + PRD updated.** Carolina's `skad-branch` delivers the **Pessoa-2 slice**: UC11 cadastro (`POST /clientes`), UC12 login (`POST /auth/login`), the real **Cozinha + Entrega simulations** (completing UC9 — full `PAGO→AGUARDANDO→PREPARACAO→PRONTO→TRANSPORTE→ENTREGUE` lifecycle, verified end-to-end), `CozinhaServiceStub` **deleted** (Seam #1), and the start of the **JPA migration** (`Cliente`→`@Entity` Approach A + `spring-boot-starter-data-jpa` + `ClienteRepositoryJPA`, Seam #3/OQ#2 resolved — see §13). Status flips: **UC9 ✅**, **Cozinha/Entrega ✅**, **P2 ✅ complete**, UC11/UC12 🟡, P3 🟡 (Pessoa-2 repos), P4 🟡. **Review (REQUEST CHANGES, posted to PR #4 + [`reviews/pr-4-review.md`](reviews/pr-4-review.md))** — empirically verified under JDK 21: **build is currently red** (`Tests run: 90, Errors: 4`) because the JPA migration left **both `ClienteRepositoryJDBC` and `ClienteRepositoryJPA` as `@Repository`** → `NoUniqueBeanDefinitionException` (app won't boot). **Two verified fixes green it:** (1) delete `ClienteRepositoryJDBC`; (2) set `spring.jpa.hibernate.ddl-auto: none` (else Hibernate `create-drop` wipes the `data.sql`-seeded clients — confirmed). Also: 3 stray `.rar` archives committed under `src/main/java`; no `UNIQUE(email)` in `schema.sql`; login returns 400 not 401; no tests cover the new auth/JPA wiring. **This commit updates the PRD only** (no code/fixes) — the fixes above are tracked for Pessoa 2 to apply on `skad-branch` before merge.
