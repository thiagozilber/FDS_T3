...
| P3 | 17/06/2026 | Persistência com JPA | Migrate JDBC repos → JPA; entities annotated | P2 | **Pessoa 2 (framework) · ambos (repos próprios)** — Seam #3 | 🟡 in-progress — **Pessoa 2 started (PR #4):** `Cliente`→JPA (Approach A, OQ#2/Seam #3 — see §13, 28/06); Pessoa 1's repos (Pedido*/Estoque) still pending — [plan](plans/jpa-migration-pessoa1-p3.plan.md) |
...
2. ~~**JPA vs pure-POJO domain.**~~ **Resolved (P3 — Seam #3, 2026-06-28, PR #4):** **Approach A — annotate the domain entities** (see §13 decision above). `ddl-auto` must be set to `none`. Pessoa 1 migrates `Pedido*`/`Estoque` against the same approach.
