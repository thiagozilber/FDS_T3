...
| UC10 | Listar pedidos entregues entre 2 datas | Cliente/Adm | 🔜 | GET `/pedidos/entregues?ini=&fim=` | ServicoPedido | Returns ENTREGUE orders in `[ini, fim]` — **planned (Pessoa 1, Seam #4)**: [plan](plans/listar-pedidos-entregues-uc10.plan.md). Filters by the `ENTREGUE` transition timestamp in `historico_status` (D15) |
