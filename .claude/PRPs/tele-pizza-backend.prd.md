...
6. **Pre-existing issues found in the P1 review (NOT introduced by P1, deferred):** (a) `GET /cardapio/{id}` and `recuperaProdutoPorid` return `null` for unknown ids → NPE/HTTP 500 instead of 404 (baseline null-return contract; consider a `NotFoundException`→404 in P2/P3); (b) `recuperaProdutosCardapio` JOIN can duplicate a produto if it ever has >1 receita (add `DISTINCT` or a UNIQUE constraint); (c) `preco` is `int` centavos but discount math is `double` — round/`BigDecimal` when applying to orders (P2) and fix the `preco bigint`-vs-`int` type mismatch (P3); (d) `@Component`/`@Repository` stereotype mix across `Adaptadores.Dados` (normalize in one sweep); (e) **security:** H2 console open + `@CrossOrigin("*")` on all endpoints + `jdbc: DEBUG` logging — lock down when auth lands (P5).
